...
public class PRTokeniser {
	...
	public static final boolean delims[] = { true, true, false, false, false,
	...
	public static final boolean isWhitespace(int ch) {
		return ch == 0 || ch == 9 || ch == 10 || ch == 12 || ch == 13
				|| ch == 32;
	}
	...
	public void throwError(String error) throws IOException {
		throw new InvalidPdfException(
				MessageLocalization.getComposedMessage("1.at.file.pointer.2",
	...
	public char checkPdfHeader() throws IOException {
		...
		if (idx < 0) {
			throw new InvalidPdfException(MessageLocalization
					.getComposedMessage("pdf.header.not.found"));
		}
	...
	public void checkFdfHeader() throws IOException {
		...
		if (idx < 0) {
			throw new InvalidPdfException(MessageLocalization
					.getComposedMessage("fdf.header.not.found"));
		}
		file.setStartOffset(idx);
	}
	...
	public int getStartxref() throws IOException {
		...
		if (idx < 0) {
			throw new InvalidPdfException(MessageLocalization
					.getComposedMessage("pdf.startxref.not.found"));
		}
		return pos + idx;
	}
	...
	public void nextValidToken() throws IOException {
		...
		// if we hit here, the file is either corrupt (stream ended
		// unexpectedly),
	...
	public boolean nextToken() throws IOException {
		...
		// Note: We have to initialize stringValue here, after we've looked for
		// the end of the stream,
		// to ensure that we don't lose the value of a token that might end
		// exactly at the end
		// of the stream
		...
		case '>':
			...
			if (ch != '>') {
				throwError(MessageLocalization
						.getComposedMessage("greaterthan.not.expected"));
			}
			type = TK_END_DIC;
			break;
		case '<': {
			...
			if (v1 < 0 || v2 < 0) {
				throwError(MessageLocalization
						.getComposedMessage("error.reading.string"));
			}
			break;
		}
		case '%':
	...
	public boolean readLineSegment(byte input[]) throws IOException {
		...
		// assumes that line provided by readLineSegment does not have init.
		// whitespace;
		if (ptr < len) {
